...
import { useCreateNewAlbumMutation, useGetAllAlbumsQuery } from "../api/api";

const Sidebar = ({ selectedPhotos, handleDeletePhotos, isDeleting, handleSharePhotos, isSharing, refreshSelectedPhotos }) => {
            ...
            <Modal className="add-to-album-modal" show={showAlbumModal} onHide={handleCloseModal}>
                ...
                <Modal.Body>
                    ...
                    <ListGroup>
                        {isSuccess &&
                            (albums.map((album) => (
                                <ListGroup.Item action key={album.id} className="album-li">{album.name}<span>{album.photos.length} elements</span></ListGroup.Item>
                            ))
                            )}
                    </ListGroup>
                </Modal.Body>
